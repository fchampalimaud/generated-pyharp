...
def profile_path(label, path, register_cls, payload_type):
    ...
    # Step 3a: DataFrame creation (no copy — zero-copy views)
    t4 = time.perf_counter()
    df = pd.DataFrame(cols, copy=False)
    t5 = time.perf_counter()
    df_no_copy_ms = (t5 - t4) * 1000
    print(f"  pd.DataFrame(copy=False): {df_no_copy_ms:.2f} ms  dtypes={list(df.dtypes)}")

    # Step 3b: DataFrame creation (copy — contiguous column copies)
    t6 = time.perf_counter()
    df_copy = pd.DataFrame(cols, copy=True)
    t7 = time.perf_counter()
    df_copy_ms = (t7 - t6) * 1000
    print(f"  pd.DataFrame(copy=True):  {df_copy_ms:.2f} ms")
    ...
    print(f"  TOTAL (copy):    {total_copy:.2f} ms")
...
for copy_label, key in [("no copy", "total_no_copy_ms"), ("copy", "total_copy_ms")]:

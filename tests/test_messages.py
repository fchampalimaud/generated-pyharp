...
def test_create_write_float():
    ...
    assert len(message.to_bytes()) == 10  # 5 header bytes + 4 float bytes + 1 checksum
...
def test_read_who_am_i() -> None:
    ...
    assert str(message.to_bytes()) == str(bytearray(b"\x01\x04\x00\xff\x02\x06"))
...
def test_create_write_U32() -> None:
    ...
    assert message.payload == value
    assert len(message.to_bytes()) == 10  # length + checksum byte
...
def test_create_write_U64() -> None:
    ...
    assert message.payload == value
    assert len(message.to_bytes()) == 14  # length + checksum byte
